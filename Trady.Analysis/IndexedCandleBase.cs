...
namespace ES.Analysis
{
    public abstract class IndexedCandleBase : CandleTrade, IIndexedOhlcv
    {
        ...
        [Obsolete]
        public IFuncAnalyzable<IAnalysis<decimal?>> GetFunc(string name, params decimal[] @params)
            => Context == null ?
            FuncAnalyzableFactory.CreateAnalyzable(name, BackingList, @params) :
            (IFuncAnalyzable<IAnalysis<decimal?>>)Context.GetFunc(name, @params);
        ...
        private TOutputType Eval<TAnalyzable, TOutputType>(params object[] @params) where TAnalyzable : IAnalyzable
        {
            ...
            if (obj.GetType().IsAssignableFrom(typeof(Analysis<TOutputType>)))
            {
                var analyzableObj = (Analysis<TOutputType>)obj;
                return analyzableObj.Tick;
            }
            else throw new TypeLoadException($"The output is not a type of Analysis<{typeof(TOutputType).Name}>");
        }
        ...
        public decimal? EvalFunc(string name, params decimal[] @params)
        {
            ...
            var analyzable = (IFuncAnalyzable<IAnalysis<decimal?>>)Context.GetFunc(name, @params);
